...
export class RunService {
  ...
  async getAllRuns() {
    try {
      ...
      logger.error('Failed to fetch all runs')
      throw error
    }
  }
  ...
  async getRunById(id: string) {
    try {
      ...
      logger.error(`Failed to fetch run ${id}`)
      throw error
    }
  }
  ...
  async getRunsByProfileId(syncProfileId: string) {
    try {
      ...
      logger.error(`Failed to fetch runs for profile ${syncProfileId}`)
      throw error
    }
  }
  ...
  async createRun(data: CreateRunData) {
    try {
      ...
      logger.info(`Created run ${run.id} for profile ${data.syncProfileId}`, run.id)
      return run
    } catch (error) {
      logger.error(`Failed to create run for profile ${data.syncProfileId}`)
      throw error
    }
  }
  ...
  async updateRun(id: string, data: UpdateRunData) {
    try {
      ...
      logger.info(`Updated run ${id}`, id)
      return run
    } catch (error) {
      logger.error(`Failed to update run ${id}`)
      throw error
    }
  }
  ...
  async startRun(id: string) {
    try {
      ...
      logger.info(`Started run ${id}`, id)
      return run
    } catch (error) {
      logger.error(`Failed to start run ${id}`)
      throw error
    }
  }
  ...
  async completeRun(id: string, status: RunStatus, exitCode?: number, errorMessage?: string) {
    try {
      ...
      logger.info(`Completed run ${id} with status ${status}`, id)
      this.runLogLevels.delete(id)
      return run
    } catch (error) {
      logger.error(`Failed to complete run ${id}`)
      throw error
    }
  }
  ...
  async getRunLogs(id: string): Promise<string[]> {
    try {
      ...
      // Create a run-specific logger to read the logs
      const runLogger = new RunLogger(id, run.logFilePath)
      return runLogger.readRunLogs(id)
    } catch (error) {
      logger.error(`Failed to read logs for run ${id}`)
      throw error
    }
  }
  ...
  createRunLogger(id: string): RunLogger {
    return logger.createRunLogger(id)
  }
  ...
  async getRecentRuns(limit: number = 50) {
    try {
      ...
      logger.error('Failed to fetch recent runs')
      throw error
    }
  }
  ...
  async getRunsByStatus(status: RunStatus) {
    try {
      ...
      logger.error(`Failed to fetch runs with status ${status}`)
      throw error
    }
  }
  ...
  async cleanupOldRuns(daysOld: number = 30) {
    try {
      ...
      for (const run of runsToDelete) {
        if (run.logFilePath && existsSync(run.logFilePath)) {
          try {
            unlinkSync(run.logFilePath)
            deletedLogFiles++
            logger.debug(`Deleted log file for run ${run.id}: ${run.logFilePath}`)
          } catch (error) {
            logger.warn(`Failed to delete log file for run ${run.id}: ${run.logFilePath}`)
          }
        }
      }
      ...
      logger.info(`Cleaned up ${result.count} old runs and ${deletedLogFiles} log files`)
      return { deletedRuns: result.count, deletedLogFiles }
    } catch (error) {
      logger.error('Failed to cleanup old runs')
      throw error
    }
  }
  ...
  async cleanupStaleRuns() {
    try {
      ...
      if (result.count > 0) {
        logger.warn(`Marked ${result.count} stale runs as failed due to timeout`)
      }
      
      return result.count
    } catch (error) {
      logger.error('Failed to cleanup stale runs')
      throw error
    }
  }
  ...
  async cancelRun(id: string) {
    try {
      ...
      logger.error(`Failed to cancel run ${id}: ${error}`)
      throw error
    }
  }
}
